...
namespace Force {
  ...
  */
  ...
  public:
    ...
    virtual bool MapAddressRange(uint64 VA, uint64 size, bool isInstr, const GenPageRequest* pPageReq) override; //!< Map virtual address range.
    virtual bool MapAddressRangeForGpa(uint64 GPA, uint64 size, bool isInstr, const GenPageRequest* pPageReq) override; //!< Map guest physical address range to host.
    ...
    const Page* GetGuestPage(uint64 GPA) const; //!< Return a pointer to the Page that covers the GPA if exists.
    ...
    TablePte* CreateGstageNextLevelTable(uint64, const PageTable* parentTable, const GenPageRequest& pPageReq); //!< Create Gstage next level page table object.
    ...
    bool GuestPhysicalMappingAvailable(uint64 start, uint64 end) const; // See if guest physical address range intersect with existing pages.
    virtual const Page* CreatePage(uint64 VA, uint64 size, GenPageRequest* pPageReq, PageSizeInfo& rSizeInfo, std::string& rErrMsg); //!< Try to create a page.
    const Page* CreateGuestPage(uint64 GPA, uint64 size, GenPageRequest* pPageReq, PageSizeInfo& rSizeInfo, std::string& rErrMsg); //!< Try to create a guest page.
  ...
  protected:
    ...
    virtual void UpdateVirtualUsableByPage(const Page* pPage); //!< Will perform an update on the virtual constraints in the pages virtual address range
    virtual Page* PageInstance() const; //!< Return an instance of the Page class or its sub-class.
    virtual const Page* MapAddress(uint64 VA, uint64 size, bool isInstr, const GenPageRequest* pPageReq, bool& newAlloc); //!< Map one virtual address.
    const Page* MapAddressForGpa(uint64 GPA, uint64 size, bool isInstr, const GenPageRequest* pPageReq, bool& newAlloc); //!< Map one guest physical address to host.
    ...
    virtual const Page* SetupPageMapping(uint64 VA, uint64 size, bool isSysPage, GenPageRequest* pPageReq); //!< Setup mapping of VA to page
    const Page* SetupPageMappingForGpa(uint64 GPA, uint64 size, bool isSysPage, GenPageRequest* pPageReq); //!< Setup mapping of GPA to page
    ...
    virtual bool AllocatePhysicalPage(uint64 VA, uint64 size, GenPageRequest* pPageReq, PageSizeInfo& rSizeInfo, EMemBankType memBank, const PagingChoicesAdapter* pChoicesAdapter); //!< Allocate physical page.
    void ConstructPageTableWalk(Page* pageObj, const GenPageRequest& pPageReq); //!< Construct page table walk.
    virtual void CommitPage(const Page* pageObj, uint64 size); //!< Commit page.
    void CommitGuestPage(const Page* pageObj, uint64 size); //!< Commit guest page.
    virtual void MapEssentialPhysicalRegions(); //!< Map essential physical regions.
  ...
  protected:
    ...
    std::vector<const Page* >     mGuestPages; //!< Sorted vector holding pointers to all guest page objects.
  ...
  */
  ...
  class GuestVmAddressSpace : public VmAddressSpace {
  public:
    Object * Clone() const override { return new GuestVmAddressSpace(*this); } //!< Clone GuestVmAddressSpace object.
    const std::string ToString() const override { return Type(); } //!< Return a string describing the VmAddressSpace object.
    const char* Type() const override { return "GuestVmAddressSpace"; }

    explicit GuestVmAddressSpace(const VmFactory* pFactory, VmasControlBlock* pVmasCtlrBlock); //!< Constructor with pointer to control block given.
    ~GuestVmAddressSpace(); //!< Destructor.
    virtual const Page* MapAddress(uint64 VA, uint64 size, bool isInstr, const GenPageRequest* pPageReq, bool& newAlloc) override;
    virtual bool AllocatePhysicalPage(uint64 VA, uint64 size, GenPageRequest* pPageReq, PageSizeInfo& rSizeInfo, EMemBankType memBank, const PagingChoicesAdapter* pChoicesAdapter) override; //!< Allocate physical page.
    virtual void CommitPage(const Page* pageObj, uint64 size) override; //!< Commit page.
    virtual void UpdateVirtualUsableByPage(const Page* pPage) override; //!< Will perform an update on the virtual constraints in the pages virtual address range
    virtual void MapEssentialPhysicalRegions() override; //!< Map guest essential physical regions.
  protected:
    GuestVmAddressSpace(); //!< Default constructor.
    GuestVmAddressSpace(const GuestVmAddressSpace& rOther); //!< Copy constructor.
  };
}
